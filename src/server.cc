    ...
    xClient* ownerClient)
{
    ...
    if (fakeClient->getNickName().empty() || fakeClient->getUserName().empty() || fakeClient->getInsecureHost().empty() || fakeClient->getDescription().empty()) {
...
{
    int hopCount = 1;
    if ((theClient->getCharYY()[0] != getCharYY()[0]) || (theClient->getCharYY()[1] != getCharYY()[1])) {
    ...
    for (; tokenIndex < st.size();) {
        ...
        for (size_t charIndex = 0; charIndex < st[tokenIndex].size();
            ...
            case 'v': {
                ...
                iClient* targetClient = Network->findNick(
                    st[nextArgIndex]);
                if (0 == targetClient) {
                    return false;
                }
    ...
    const string& message)
{
    ...
    if (msgTokens[0] == "loadclient") {
        ...
        Notice(srcClient, string("Attempting to load client module: ") + msgTokens[1] + ", with config file: " + msgTokens[2]);
        return;
    }
}
