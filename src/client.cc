    ...
    const std::vector<iClient*>& clientVector)
{
    ...
    for (xServer::banVectorType::const_iterator ptr = banVector.begin(),
        ...
        if (((MAX_CHAN_MODES + 1) == modeString.size()) || ((ptr + 1) == end)) {
            stringstream s;
            s << getCharYYXXX() << " M "
              << theChan->getName() << ' '
              << modeString << ' ' << args;
...
{
    Channel* theChan = Network->findChannel(chanName);
    return theChan ? Join(theChan, chanModes, joinTime, getOps) : MyUplink->JoinChannel(this, chanName, chanModes, joinTime, getOps);
}
