...
void stats::dumpStats(iClient* theClient)
{
    ...
    usersPerChannelFile << "Channel size\tNumber of channels of that size"
                        << endl;
    for (channelUserInfoMapType::const_iterator itr = usersPerChannelMap.begin(); itr != usersPerChannelMap.end();
         ++itr) {
    ...
    for (channelUserInfoMapType::const_iterator itr = channelsPerUserMap.begin(); itr != channelsPerUserMap.end();
         ++itr) {
