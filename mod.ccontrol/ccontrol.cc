...
*/
	...
	return ;
        ...
        if ((!theUser) && !(ComAccess & commandLevel::flg_NOLOGIN) && (ComAccess)) { //The user isnt authenticated,
        ...
        void* Data1, void* Data2, void* Data3, void* Data4)
    {
		 ...
		 */
            ...
            iClient* tmpUser = (theEvent == EVT_QUIT) ? static_cast<iClient*>(Data1) : static_cast<iClient*>(Data2);
            --curUsers;
            ...
            if ((checkClones) && (irc_in_addr_valid(&tmpUser->getIP()))) //avoid 0:: (0.0.0.0) ip addresses
            {
                ...
                if ((clientsIp24Map.find(client_ip) != clientsIp24Map.end()) && (--clientsIp24Map[client_ip] < 1)) {
                    clientsIp24Map.erase(clientsIp24Map.find(client_ip));
                    if ((clientsIp24MapLastWarn.find(client_ip) != clientsIp24MapLastWarn.end()) && (clientsIp24MapLastWarn[client_ip] > 0))
                ...
                if ((clientsIp24IdentMap.find(Log) != clientsIp24IdentMap.end()) && (--clientsIp24IdentMap[Log] < 1)) {
                    clientsIp24IdentMap.erase(clientsIp24IdentMap.find(Log));
                    if ((clientsIp24IdentMapLastWarn.find(Log) != clientsIp24IdentMapLastWarn.end()) && (clientsIp24IdentMapLastWarn[Log] > 0))
                ...
                if ((virtualClientsMap.find(virtualHost) != virtualClientsMap.end()) && (--virtualClientsMap[virtualHost] < 1)) {
                    virtualClientsMap.erase(virtualClientsMap.find(virtualHost));
                    if ((virtualClientsMapLastWarn.find(virtualHost) != virtualClientsMapLastWarn.end()) && (virtualClientsMapLastWarn[virtualHost] > 0))
 ...
 */
    void ccontrol::handleNewClient(iClient* NewUser)
    {
        ...
        if (dbConnected) {
			...
			else
			{
                ...
                if ((CurIdentConnections > maxIClones) && (CurIdentConnections > getExceptions(NewUser->getUserName() + "@" + tIP)) && (CurIdentConnections > getExceptions(NewUser->getUserName() + "@" + NewUser->getRealInsecureHost()))) {
                ...
                if (!SKIPTHIS)
                    if (irc_in_addr_is_ipv4(&NewUser->getIP()))
                        if ((!isShellException) && (CurCIDRConnections > maxCClones) && (CurCIDRConnections > getExceptions(NewUser->getUserName() + "@" + tIP)) && (CurCIDRConnections > getExceptions(NewUser->getUserName() + "@" + NewUser->getRealInsecureHost()))) {
			...
			*/
                ...
                if (!StdCloneChecksDisabled)
                    if ((!isShellException) && (CurConnections > maxClones) && (CurConnections > getExceptions(NewUser->getUserName() + "@" + client_ip)) && (CurConnections > getExceptions(NewUser->getUserName() + "@" + NewUser->getRealInsecureHost())) && (!DoGline)) {
                ...
                if ((DoGline) && (shellglinecounter == 0)) {
                    ...
                    if ((CurConnections > maxVClones) && (CurConnections > getExceptions("*@" + createClass(tIP)))) //getExceptions("*@" + ipClass)))
                    {
                        ...
                        if ((virtualClientsMapLastWarn[virtualHost] + CClonesTime) <= time(NULL)) {
                            ...
                            MsgChanLog("Virtual clones for real name %s on %s, total connections %d\n",
                                NewUser->getDescription().c_str(), ipClass.c_str(), CurConnections);
	...
	if((!glSet)) 
        ...
        while (tIterator != usersMap.end()) {
            ...
            if ((tUser->getLastAuthTS() == NewUser->getConnectTime()) && (tUser->getLastAuthNumeric() == NewUser->getCharYYXXX())) {
    ...
    void ccontrol::addGlineToUplink(ccGline* theGline)
    {
        ...
        MyUplink->setGline(theGline->getAddedBy(), theGline->getHost(), theGline->getReason(), Expires, theGline->getLastUpdated(), this);
    }
	...
	*/
    ...
    bool ccontrol::glineChannelUsers(iClient* theClient, Channel* theChan, const string& reason, unsigned int gLength, const string& addedBy, bool excludeChanWithOper, bool unidented)
    {
        ...
        for (Channel::const_userIterator ptr = theChan->userList_begin();
            ...
            if ((excludeChanWithOper) && (TmpClient->isOper() || TmpClient->getMode(iClient::MODE_SERVICES))) //Need to stop glining this channel
            {
                foundOper = true;
                success = false;
                break;
            ...
            if ((!TmpClient->getMode(iClient::MODE_SERVICES)) && !(IsAuth(TmpClient)) && !(TmpClient->isOper())) {
