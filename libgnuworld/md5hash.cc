...
void md5::report(md5Digest& digest)
...
void md5::encode(unsigned char* output, const uint32_t* input, int32_t len)
...
void md5::encode(md5Digest& digest)
...
void md5::decode(uint32_t* output, const unsigned char* input, int32_t len)
{ // Decodes input (unsigned char) into output (uint32_t).
    // Assumes len is a multiple of 4.
    int32_t ii, jj;
    for (ii = 0, jj = 0; jj < len; ii++, jj += 4) {
        output[ii] = ((uint32_t)input[jj]) | (((uint32_t)input[jj + 1]) << 8) | (((uint32_t)input[jj + 2]) << 16) | (((uint32_t)input[jj + 3]) << 24);
    }
    return;
}
