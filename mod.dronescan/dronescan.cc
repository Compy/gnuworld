 ...
 */
        ...
        if (!MyUplink->UnRegisterTimer(tidClearJoinCounter, 0) || !MyUplink->UnRegisterTimer(tidClearNickCounter, 0) || !MyUplink->UnRegisterTimer(tidClearActiveList, 0)) {
 ...
 *******************************************/
...
#endif
                    if ((joinPartIt->second.numOfJoins >= jcMinJoinToGline && joinPartIt->second.numOfParts >= jcMinJoinToGline) || (joinPartIt->second.numOfJoins >= jcMinJoinToGlineJOnly && jChannel->getNumOfJoins() >= jcMinJFJOnlySizeToGline)) {
                        if (!joinPartIt->second.seenOper && !joinPartIt->second.seenLoggedInUser) {
                            ...
                            for (; numericsIt != joinPartIt->second.numerics.end(); ++numericsIt) {
                                ...
                                if (theClient && !strcmp(xIP(theClient->getIP()).GetNumericIP().c_str(), joinPartIt->first.c_str())) {
                                    nickCount++;
                                    if (nickCount > 1)
                                        tempNames << ",";
    ...
    void dronescan::setNickStates()
    {
        ...
        for (xNetwork::const_clientIterator ptr = Network->clients_begin(); ptr != Network->clients_end(); ++ptr) {
	 ...
	 */
        if (userEntropy > averageEntropy * (1 + nickMargin) || userEntropy < averageEntropy * (1 - nickMargin))
            return theData->setState(ABNORMAL);
