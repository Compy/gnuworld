...
namespace ds {
    ...
    void LISTCommand::Exec(const iClient* theClient, const string& Message, const sqlUser*)
    {
        ...
        if ("ACTIVE" == Name) {
            ...
            for (dronescan::droneChannelsType::const_iterator itr = bot->droneChannels.begin(); itr != bot->droneChannels.end(); ++itr) {
        ...
        if ("FAKECLIENTS" == Name) {
            ...
            for (dronescan::fcMapType::const_iterator itr = bot->fakeClients.begin(); itr != bot->fakeClients.end();
                 ++itr) {
        ...
        if ("JOINFLOOD" == Name) {
            ...
            for (dronescan::jcChanMapType::const_iterator itr = bot->jcChanMap.begin(); itr != bot->jcChanMap.end(); ++itr) {
        ...
        if ("EXCEPTIONALCHANNELS" == Name) {
            ...
            for (dronescan::exceptionalChannelsType::const_iterator itr = bot->exceptionalChannels.begin(); itr != bot->exceptionalChannels.end(); ++itr) {
