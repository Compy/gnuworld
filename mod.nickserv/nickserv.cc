 ...
 */
   ...
   * has AUTOKILL set. Warn them. */
            ...
            if (theData->warned) {
                ...
                theLogger->log(logging::events::E_INFO, "Warned: (" + theClient->getCharYYXXX() + ") " + theClient->getNickName());
                queuePos++;
                continue;
            } // if(theData->warned)
        ...
        for (QueueType::iterator queuePos = killQueue.begin(); queuePos != killQueue.end(); queuePos++) {
            ...
            theLogger->log(logging::events::E_INFO, "Killed: (" + theClient->getCharYYXXX() + ") " + theClient->getNickName());
