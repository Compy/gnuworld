        ...
        if (match(matchString, bot->SQLDb->GetValue(i, 1)) == 0) {
            ...
            if (suspend_expires > bot->currentTime()) {
                bot->Notice(theClient,
                    ...
                    suspendLevel);
                bot->Notice(theClient, bot->getResponse(theUser, language::susp_reason, string("Reason: %s")).c_str(), suspReason.c_str());
            } else if ((suspReason != "") && (suspend_expires == 0))
                bot->Notice(theClient, bot->getResponse(theUser, language::unsusp_reason, string("UNSUSPENDED - %s")).c_str(), suspReason.c_str());
            bot->Notice(theClient,
                bot->getResponse(theUser,
                       language::last_seen)
                    .c_str(),
