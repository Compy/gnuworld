...
namespace cf {
    ...
    void CHANFIXCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message)
    {
        ...
        if (bot->getState() != chanfix::RUN) {
            bot->SendTo(theClient,
                bot->getResponse(theUser,
                       ...
                       std::string("Sorry, chanfix is currently disabled because not enough servers are linked."))
                    .c_str());
            return;
        }
        ...
        if (theChan->getMaxScore() <= static_cast<int>(static_cast<float>(FIX_MIN_ABS_SCORE_END) * MAX_SCORE)) {
            bot->SendTo(theClient,
                bot->getResponse(theUser,
                       language::highscore_channel,
                       std::string("The highscore in channel %s is %d which is lower than the minimum score required (%.2f * %d = %d)."))
                    .c_str(),
            ...
            if (!override) {
                bot->SendTo(theClient,
                    bot->getResponse(theUser,
                           ...
                           std::string("The channel %s is being automatically fixed. Append the OVERRIDE flag to force a manual fix."))
                        .c_str(),
                    netChan->getName().c_str());
                return;
            } else {
        ...
        if (bot->isTempBlocked(theChan->getChannel()) && !override) {
            bot->SendTo(theClient,
                bot->getResponse(theUser,
                       ...
                       std::string("The channel %s is TEMPBLOCKED. To fix it, please append the OVERRIDE flag."))
                    .c_str(),
                theChan->getChannel().c_str());
            return;
        }
        ...
        if (theChan->getFlag(sqlChannel::F_ALERT) && !override) {
            bot->SendTo(theClient,
                bot->getResponse(theUser,
                       ...
                       std::string("Alert: The channel %s has notes. Use \002INFO %s\002 to read them. Append the OVERRIDE flag to force a manual fix."))
                    .c_str(),
