        ...
        if (string_lower(user_name) != string_lower(theUser->getUserName()) && !theUser->getFlag(sqlcfUser::F_USERMANAGER)) {
            bot->SendTo(theClient,
                bot->getResponse(theUser,
                       ...
                       std::string("Note #%d for channel %s was not added by you. You can only delete notes that you added."))
                    .c_str(),
                messageId, theChan->getChannel().c_str());
            return;
        }
        ...
        if (eventType != sqlChannel::EV_NOTE && !theUser->getFlag(sqlcfUser::F_OWNER)) {
            bot->SendTo(theClient,
                bot->getResponse(theUser,
                       ...
                       std::string("Note #%d for channel %s is not a manually added note. You can only delete notes that were manually added."))
                    .c_str(),
                messageId, theChan->getChannel().c_str());
            return;
        }
