            ...
            if (!theUser->matchHost(theClient->getRealNickUserHost().c_str())) {
                SendTo(theClient,
                    getResponse(theUser,
                        ...
                        std::string("Your current host does not match any registered hosts for your username."))
                        .c_str());
                return;
            }
#endif
 ...
 */
        ...
        for (xNetwork::channelIterator ptr = Network->channels_begin(); ptr != Network->channels_end(); ptr++) {
            ...
            if (thisChan->size() >= minClients && !isBeingFixed(thisChan)) {
                ...
                if (opLess && !hasService) {
                    ...
                    if ((sqlChan->getMaxScore() > static_cast<int>(static_cast<float>(FIX_MIN_ABS_SCORE_END)
                             * MAX_SCORE))
    ...
    bool chanfix::simFix(sqlChannel* sqlChan, bool autofix, time_t c_Time, iClient* theClient, sqlcfUser* theUser)
    {
        ...
        int min_score_abs = static_cast<int>((MAX_SCORE * static_cast<float>(FIX_MIN_ABS_SCORE_BEGIN)) - static_cast<float>(time_since_start) / static_cast<float>(max_time) * (MAX_SCORE * static_cast<float>(FIX_MIN_ABS_SCORE_BEGIN) - static_cast<float>(FIX_MIN_ABS_SCORE_END) * MAX_SCORE));

        int min_score_rel = static_cast<int>((maxScore * static_cast<float>(FIX_MIN_REL_SCORE_BEGIN)) - static_cast<float>(time_since_start) / static_cast<float>(max_time) * (maxScore * static_cast<float>(FIX_MIN_REL_SCORE_BEGIN) - static_cast<float>(FIX_MIN_REL_SCORE_END) * maxScore));
        ...
        if ((!numClientsToOp || maxScore < min_score) && (!autofix || !(numClientsToOp + currentOps))) {
            if (autofix && !sqlChan->getSimModesRemoved()) {
                ...
                if (netChan->banList_size() || netChan->getMode(Channel::MODE_I) || netChan->getMode(Channel::MODE_K) || netChan->getMode(Channel::MODE_L) || netChan->getMode(Channel::MODE_R) || netChan->getMode(Channel::MODE_D)) {
        ...
        if (numClientsToOp) {
            SendTo(theClient,
                getResponse(theUser,
                    language::sim_opping,
                    std::string("(%s) Opping: %s (%d Clients)"))
                    .c_str(),
 ...
 * for now.
 */
        if (theChan->banList_size() || theChan->getMode(Channel::MODE_I) || theChan->getMode(Channel::MODE_K) || theChan->getMode(Channel::MODE_L) || theChan->getMode(Channel::MODE_R) || theChan->getMode(Channel::MODE_D))
            return true;
...
int chanfix::getNewScore(sqlChanOp* chOp, time_t oldestTS)
{
    ...
    elog << "chanfix::getNewScore> daysSinceFirstOpOnChan: "
         << daysSinceFirstOpOnChan << " daysSinceFirstOp: "
         << daysSinceFirstOp
         << " x = "
         << x
