            ...
            if (bot->isBeingChanFixed(netChan)) {
                bot->SendTo(theClient,
                    bot->getResponse(theUser,
                           ...
                           std::string("WARNING: Channel %s is being manually fixed; aborting fix as per BLOCK."))
                        .c_str(),
                    theChan->getChannel().c_str());
            ...
            if (bot->isBeingAutoFixed(netChan)) {
                bot->SendTo(theClient,
                    bot->getResponse(theUser,
                           ...
                           std::string("WARNING: Channel %s is being automatically fixed; aborting fix as per BLOCK."))
                        .c_str(),
                    theChan->getChannel().c_str());
                bot->stopFixingChan(netChan, true);
