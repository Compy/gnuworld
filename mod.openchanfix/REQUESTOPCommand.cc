 ...
 */
    void REQUESTOPCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message)
    {
        ...
        if (bot->getState() != chanfix::RUN) {
            bot->SendTo(theClient,
                bot->getResponse(theUser,
                       ...
                       std::string("Sorry, chanfix cannot fix channels at the moment, please try again soon."))
                    .c_str());
            return;
        }
        ...
        if (!isRequesterScoreHighEnough) {
            bot->SendTo(theClient,
                bot->getResponse(theUser,
                       ...
                       std::string("You score for %s is not high enough to issue a fix."))
                    .c_str(),
                netChan->getName().c_str());
            return;
        }
        ...
        if (mintime < bot->getMinRequestOpTime()) {
            ...
            bot->SendTo(theClient,
                bot->getResponse(theUser,
                       ...
                       std::string("You must wait %s seconds before %s can be fixed again with this command, or contact a help channel."))
                    .c_str(),
        ...
        if (bot->isBeingAutoFixed(netChan)) {
            bot->SendTo(theClient,
                bot->getResponse(theUser,
                       ...
                       std::string("The channel %s is being automatically fixed. You cannot issue another fix at the moment."))
                    .c_str(),
                netChan->getName().c_str());
            return;
        }
        ...
        if ((bot->isTempBlocked(theChan->getChannel())) || (theChan->getFlag(sqlChannel::F_BLOCKED)) || (theChan->getFlag(sqlChannel::F_ALERT))) {
            bot->SendTo(theClient,
                bot->getResponse(theUser,
                       language::channel_blocked_non_oper,
                       std::string("The channel %s cannot be fixed, please contact a help channel for further information."))
                    .c_str(),
        ...
        bot->SendTo(theClient,
            bot->getResponse(theUser,
                   ...
                   std::string("Fixing %s, please wait."))
                .c_str(),
            netChan->getName().c_str());
